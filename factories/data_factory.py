...
def get_flickr8k_data_with_spatial_graphs(
    root_folder,
    annotation_file,
    transform,
    graph_dir,
    ...
    dataset = Flickr8kDatasetWithSpatialGraphs(root_folder, annotation_file, transform=transform, graph_dir=graph_dir)
    pad_idx = dataset.vocab.stoi["<PAD>"]
    train_size = int(train_ratio * len(dataset))
    test_size = len(dataset) - train_size
    ...
    train_loader = DataLoader(
        ...
        shuffle=shuffle,
        pin_memory=pin_memory,
        collate_fn=Flickr8kGraphsBatcher(pad_idx=pad_idx),
    )
