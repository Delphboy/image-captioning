...
class Attention(nn.Module):
    ...
    def forward(self, visual_features, attn_out):
        ...
        a = self.tanh(a_v + a_h)
        a = self.a_att(a)
        a_weights = F.softmax(a)
        v_hat = torch.sum(a_weights * visual_features, dim=1).unsqueeze(2)
        return v_hat
...
class DualLstm(nn.Module):
    def __init__(self, args, vocab):
        ...
        self.lstm_attn = nn.LSTMCell(self.token_dim, self.token_dim)
        self.lstm_lang = nn.LSTMCell(self.token_dim, self.token_dim)
    ...
    def init_hidden_state(self):
        ...
        """
        h = torch.zeros(self.batch_size, self.token_dim)
        c = torch.zeros(self.batch_size, self.token_dim)
        return h, c
    ...
    def forward(self, features, caption):
        ...
        h_attn, c_attn = self.init_hidden_state()
        h_lang, c_lang = self.init_hidden_state()
        ...
        for t in range(self.max_len):
            ...
            x = torch.cat([h_attn, self.full_attn(features, h_attn)])
